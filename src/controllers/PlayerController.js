...
class Player {
  async createPlayer(req, res) {
    ...
    const isExistingNumber = await playerService.findByPlayerNumber(
      req.body.player_number
    );
    if (isExistingNumber)
      throw new DuplicateError("Player with number alredy exist");
    ...
    if (!newPlayer)
      throw new InternalServerError(
        "Something went wrong in registering player"
      );
  ...
  async deletePlayer(req, res) {
    const playerDeleted = await playerService.findPlayerAndDelete(
      req.params.id,
      req.club._id
    );
    if (!playerDeleted)
      throw new InternalServerError(
        "something went wrong in de-registering a player"
      );
    req.club.number_of_players -= 1;
    await req.club.save();
    res
